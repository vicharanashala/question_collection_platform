...
| 2026-08-12 | 8 | 12 | `develop` migrated the backend from PostgreSQL/TypeORM to MongoDB/Mongoose (see `test_plan.md`'s 2026-08-12 section). Rewrote this suite's `DataSource` usage onto the repository abstraction; replaced the removed `QuestionStatus.HUMAN_REVIEW` with `HELD`. 12 tests fail on two newly-found, precisely root-caused systemic bugs (not the old T12/T17 findings, which are separate and still apply on their own terms — see below). |

**Two systemic real bugs, not fixed, explain T1–T4, T6, T7, T10, T11, T13, T14, T16, T20:**

1. **TypeORM `Between()` FindOperator is mistranslated to a MongoDB filter.** `mongo-utils.ts`'s `translateValue()` checks `isTypeormFindOperator(val)` *before* `isTypeormBetween(val)` — and `Between()`'s real FindOperator has `_type: 'between'` with the `@instanceof: Symbol(FindOperator)` marker (verified directly: `Between(a,b)._type === 'between'`), so it always matches the first check and never reaches the dedicated Between handler. Inside that first branch, the `typeToMongo` map has no `'between'` entry (only `moreThanOrEqual`/`lessThanOrEqual`/`moreThan`/`lessThan`/`equal`/`notEqual`), so it falls through to `return val._value` — the raw `[from, to]` array — which becomes `{ submittedAt: [from, to] }` in the Mongo query: "submittedAt equals this literal array," which no document ever matches. Every date-range-filtered analytics query (dashboard/questions/rewards stats, CSV export) silently returns zero rows for the filtered portion. The same bug affects `In()` too (used for id-list filters elsewhere) — same interception, same missing map entry, same raw-array fallthrough.
2. **Raw SQL / multi-condition join strings passed into `createQueryBuilder()`** (e.g. `audit.service.ts`'s `queryAuditLogs()`: `.leftJoin('users', 'u', 'al.actor_id = u.id AND al.actor_type IN (:...types)', ...)`) can't be parsed by `MongoQueryBuilder`, which only recognizes a small set of single-condition regex patterns (`field = :param`, `field IN (:...params)`, `field BETWEEN`, `field >=`, etc.) — not compound AND/OR conditions or real SQL. Same root cause as `UserProfile.e2e.md`'s leaderboard finding. Silently breaks the actor-name join and, transitively, most of the audit-log query/stats/entity-lookup endpoints.

T7 (excel export 500) is a related but distinct symptom: `AdminService.exportData()` hands `json2xls` a malformed/empty row set (upstream of the same class of query bug), and `json2xls` crashes trying to read `Object.keys(data[0])` on `undefined`.

**Older, Postgres-era findings, still real but the mechanism has shifted:**

- **T12** (`role=curator` filter always empty — `actorTypeForRole()` only special-cases `'finance'`): unchanged, still applies on its own terms.
- **T17** (`users-by-role` 500): the originally-documented cause (`dataSource.getRepository('user')`, lowercase-string mismatch) is now **superseded** — `getUsersByRole()` (`audit.service.ts:538`) still calls `this.ds.getRepository('user')` directly, and `this.ds` (an `@Optional()` TypeORM `DataSource`) is never provided at all in Mongo mode, so it throws before the lowercase-name issue would ever matter. Observable outcome (500) is unchanged, just the specific cause underneath it.
