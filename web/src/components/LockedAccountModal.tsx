...
import { accountLockedEmitter, type AccountLockedInfo } from '@/events/accountLockedEvents'
      ...
      >
        <DialogHeader className="items-center text-center">
          <div className={cn(
            ...
            isBan ? 'bg-red-100 dark:bg-red-900/50' : 'bg-amber-100 dark:bg-amber-900/50',
          )}>
            {isBan
          ...
          <DialogDescription className="text-center text-muted-foreground">
            Your access has been revoked. Sign out to return to the login screen.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className={cn('rounded-lg border p-4 space-y-3', bgCls)}>
          {lockedInfo.reason && (
            <div>
              ...
              <p className="text-sm text-foreground italic">&ldquo;{lockedInfo.reason}&rdquo;</p>
            </div>
          )}
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground mb-1">
            ...
            <p className="text-sm text-foreground">
              ...
              {!isBan && lockedInfo.suspendedUntil && (
                <span className="text-muted-foreground"> — until {formatDate(lockedInfo.suspendedUntil)}</span>
              )}
            </p>
          </div>
        </div>
