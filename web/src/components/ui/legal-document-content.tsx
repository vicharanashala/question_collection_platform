...
export function LegalDocumentContent({ type, showHeader = true }: LegalDocumentContentProps) {
              ...
              <div key={id} className="rounded-lg border border-border-subtle overflow-hidden">
                ...
                >
                  <span className="flex flex-1 items-center gap-2.5">
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-[11px] sm:text-xs font-bold text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300">
                      {id}
                    </span>
                    <span className="text-xs sm:text-sm font-medium text-foreground">{sectionTitle}</span>
                  </span>
                  {isOpen
                ...
                {isOpen && (
                  <div className="border-t border-border-subtle px-3 py-2.5">
                    <p className="text-[11px] sm:text-xs leading-relaxed text-muted-foreground">{body}</p>
                  </div>
                )}
              </div>
            )
