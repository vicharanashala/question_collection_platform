...
function StepOtp({
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>, idx: number) => {
    if (e.key === "Backspace" && otp[idx] === "" && idx > 0) {
      ...
      const prev = inputRef.current?.parentElement?.children[idx - 1] as
        | HTMLInputElement
        | undefined;
      prev?.focus();
    }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="flex items-center gap-2 rounded-lg border border-[hsl(var(--border-subtle))] bg-[hsl(var(--surface-variant))] px-3 py-2.5">
        ...
        <span className="text-xs font-medium text-[hsl(var(--text))] flex-1">
          +91 {mobile}
        </span>
        <button
          type="button"
...
export function LoginPage() {
  ...
  async function handleSendOtp(e: React.FormEvent) {
    ...
    if (!/^[6-9]\d{9}$/.test(cleaned)) {
      toast.error("Mobile number must be a valid phone number");
      return;
    }
    setLoading(true);
    try {
