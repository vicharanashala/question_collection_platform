...
export function QuestionsPage() {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(apiTotal / (view === "card" ? cardLimit : limit)),
  );
  ...
  const columns: ColumnDef<Question>[] = [
    ...
    },
    {
      ...
      render: (q) => (
        <span
          ...
          title={q.user?.name ?? q.userName ?? q.userMobileNumber ?? ""}
        >
    ...
    },
    {
      ...
      render: (q) => (
        <span className="text-[11px] sm:text-[11px] sm:text-xs capitalize">
          {SEASON_LABEL[q.season] ?? q.season ?? "—"}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      sortable: true,
      render: (q) => (
        <span className="text-[11px] sm:text-[11px] sm:text-xs truncate block">
          {q.reviewedByName ?? "—"}
        </span>
      ),
    },
    {
      ...
      render: (q) =>
        q.mediaUrls && q.mediaUrls.length > 0 ? (
          <span className="flex items-center gap-1 text-[11px] sm:text-[11px] sm:text-xs text-muted-foreground">
  ...
  // ─── Status filter chips ───────────────────────────────────────────────────
  const statusOptions: {
    value: "held" | "approved" | "rejected" | "pending" | "";
    label: string;
  }[] = [
    { value: "", label: "All" },
    ...
    <div className="mx-auto max-w-5xl px-4 sm:px-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3">
          {apiTotal > 0 && (
            <span className="text-[11px] sm:text-[11px] sm:text-xs text-muted-foreground hidden sm:block">
              {apiTotal.toLocaleString()} total
            </span>
          )}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-x-12 gap-y-2.5">
                  ...
                  <InfoRow
                    icon={Globe}
                    label="Language"
                    value={detailQuestion.language?.toUpperCase()}
                  />

                  <InfoRow
                    icon={Info}
                    label="Category"
                    value={(detailQuestion.domains ?? []).join(", ") || "—"}
                  />

                  <InfoRow
                    icon={Wheat}
                    label="Crop Type"
                    value={detailQuestion.cropType}
                  />
                  ...
                  <InfoRow
                    ...
                    label="Season"
                    value={
                      SEASON_LABEL[detailQuestion.season] ??
                      detailQuestion.season
                    }
                  />
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-x-16 gap-y-4">
                  <InfoRow
                    ...
                    label="Submitted By"
                    value={
                      detailQuestion.user?.name ??
                      detailQuestion.userName ??
                      "—"
                    }
                  />
                  ...
                  <InfoRow
                    icon={Clock}
                    label="Submitted"
                    value={formatDate(detailQuestion.submittedAt)}
                  />

                  {detailQuestion.reviewedAt && (
                    <InfoRow
                      icon={Clock}
                      label="Reviewed"
                      value={formatDate(detailQuestion.reviewedAt)}
                    />
                  )}

                  {detailQuestion.reviewedByName && (
                    <InfoRow
                      icon={User}
                      label="Reviewed By"
                      value={detailQuestion.reviewedByName}
                    />
                  )}
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {detailQuestion.approvalReason && (
                      <InfoRow
                        ...
                        value={detailQuestion.approvalReason}
                      />
                    )}
                    ...
                    {detailQuestion.rejectionReason && (
                      <InfoRow
                        ...
                        value={detailQuestion.rejectionReason}
                      />
                    )}
                    ...
                    {detailQuestion.heldReason && (
                      <InfoRow
                        icon={PauseCircle}
                        label="Hold Reason"
                        value={detailQuestion.heldReason}
                      />
                    )}
                  </div>
                </div>
